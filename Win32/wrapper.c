...
DWORD threadCreate(LPTHREAD_START_ROUTINE threadFunc, LPVOID threadParams) {
	...
	HANDLE hThread = CreateThread(NULL, 0, threadFunc, threadParams, 0,
		&dwThreadId);
...
HANDLE mailslotCreate(char *name) {
	...
	char path[255] = "\\\\.\\mailslot\\";

	HANDLE hSlot = CreateMailslot(lstrcat(path, name), DEFAULT_STACK_SIZE, TIME_OUT,
...
HANDLE mailslotConnect(char * name) {
	...
	HANDLE hFile = CreateFile(lstrcat(path, name), GENERIC_WRITE, FILE_SHARE_READ,
...
int mailslotWrite(HANDLE mailSlot, void *msg, int msgSize) {
	...
	fResult = WriteFile(mailSlot, msg, msgSize, &cbWritten,
		(LPOVERLAPPED)NULL);
...
int	mailslotRead(HANDLE mailbox, void *msg, int msgSize) {
	...
	fResult = ReadFile(mailbox, msg, msgSize, &cbRead, (LPOVERLAPPED)NULL);
